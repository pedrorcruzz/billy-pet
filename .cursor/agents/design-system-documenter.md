...
2. **Ler** `constants/Tokens.ts`, `constants/Colors.ts`, `.cursor/rules/design-system.mdc`
3. **Montar** o documento seguindo exatamente a estrutura da skill (seções 1 a 6)
4. **Incluir** tabela de tokens, paleta Colors.ts, resumo prático
5. **Criar** o arquivo em `docs/design-system/` com versionamento (v1.md, v2.md, v3.md...)
...
**Após as seções 1–6**: incluir Paleta de Cores (Colors.ts), Tokens (Tokens.ts), Resumo Prático.
...
- **Não inventar**: usar apenas os valores de Tokens.ts, Colors.ts e design-system.mdc
